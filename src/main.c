...
// Direction: | 0: null | 1:left | 2: right | 3:up | 4:down
...
UINT8 gb_rand(UINT8 max) {
    rng_seed = (rng_seed * 1103515245 + 12345) & 0xFFFF; // LCG 16 bits
    return (rng_seed % max);
}
...
void move(int direction) {
    ...
    if (fruit.active && headX == fruit.x && headY == fruit.y) {
        score++;
        snake_length++;     // le serpent grandit
        fruit.active = 0;   // fruit mangé
        spawn_fruit();      // générer un nouveau fruit
    }
    ...
    if (!fruit.active) spawn_fruit();
}

void update_score() {
    if (score == prevScore) return;

    UINT8 tens = score / 10;  // tens
    UINT8 ones = score % 10;  // ones

    // tens
    set_win_tile_xy(2, 0, digitTiles[tens]);

    // ones
    set_win_tile_xy(3, 0, digitTiles[ones]);
...
void spawn_fruit() {
    ...
    // Tile du fruit = 15 (assure-toi que le tile existe et représente le fruit)
